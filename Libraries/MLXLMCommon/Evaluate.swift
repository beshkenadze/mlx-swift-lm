...
public struct GenerateParameters: Sendable {
    ...
    ///
    /// `nil` lets each model pick its own prefill chunk size (e.g. the Gemma 3 text
    /// path is tuned to a smaller chunk than the generic 512-token default). Set an
    /// explicit value to override the model's choice.
