...
public class LLMRegistry: AbstractModelRegistry, @unchecked Sendable {
    ...
    // TranslateGemma: translation-tuned fine-tunes of Gemma 3 (vision tower removed),
    // loaded through the existing `gemma3` text path. Translation is driven by an
    // opinionated chat template: pass the source/target languages per request via
    // `UserInput.additionalContext` (`source_lang_code` / `target_lang_code`, ISO 639-1).
    // See `Gemma3MessageGenerator` in `Gemma3Text.swift`. `defaultPrompt` is just the text
    // to translate.
